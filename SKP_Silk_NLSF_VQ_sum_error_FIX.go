...
func SKP_Silk_NLSF_VQ_sum_error_FIX(err_Q20 *int32, in_Q15 *int32, w_Q6 *int32, pCB_Q15 *int16, N int32, K int32, LPC_order int32) {
	...
	for m = 0; int64(m) < (int64(LPC_order) >> 1); m++ {
		Wcpy_Q6[m] = int32(int64(*(*int32)(unsafe.Add(unsafe.Pointer(w_Q6), unsafe.Sizeof(int32(0))*uintptr(int64(m)*2)))) | int64(*(*int32)(unsafe.Add(unsafe.Pointer(w_Q6), unsafe.Sizeof(int32(0))*uintptr(int64(m)*2+1))))<<16)
	}
	for n = 0; int64(n) < int64(N); n++ {
		cb_vec_Q15 = pCB_Q15
		for i = 0; int64(i) < int64(K); i++ {
			sum_error = 0
			for m = 0; int64(m) < int64(LPC_order); m += 2 {
				Wtmp_Q6 = Wcpy_Q6[int64(m)>>1]
				...
				diff_Q15 = int32(int64(*(*int32)(unsafe.Add(unsafe.Pointer(in_Q15), unsafe.Sizeof(int32(0))*uintptr(int64(m)+1)))) - int64(*func() *int16 {
					p := &cb_vec_Q15
					x := *p
					*p = (*int16)(unsafe.Add(unsafe.Pointer(*p), unsafe.Sizeof(int16(0))*1))
					return x
