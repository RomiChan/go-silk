...
func SKP_Silk_pitch_analysis_core(signal []int16, pitch_out []int32, lagIndex *int32, contourIndex *int32, LTPCorr_Q15 *int32, prevLag int32, search_thres1_Q16 int32, search_thres2_Q15 int32, Fs_kHz int32, complexity int32, forLJC int32) int32 {
	...
	for k = 0; k < 2; k++ {
		SKP_assert(uintptr(unsafe.Pointer(&target_ptr[0])) >= uintptr(unsafe.Pointer(&signal_4kHz[0])))
		SKP_assert(uintptr(unsafe.Pointer(&target_ptr[sf_length_8kHz])) <= uintptr(unsafe.Pointer(&signal_4kHz[frame_length_4kHz])))
		basis_ptr = ([]int16)((*int16)(unsafe.Add(unsafe.Pointer(&target_ptr[0]), -int(unsafe.Sizeof(int16(0))*uintptr(min_lag_4kHz)))))
		SKP_assert(uintptr(unsafe.Pointer(&basis_ptr[0])) >= uintptr(unsafe.Pointer(&signal_4kHz[0])))
		SKP_assert(uintptr(unsafe.Pointer(&basis_ptr[sf_length_8kHz])) <= uintptr(unsafe.Pointer(&signal_4kHz[frame_length_4kHz])))
		normalizer = 0
		cross_corr = 0
	...
	for i = max_lag_4kHz; i >= min_lag_4kHz; i-- {
		...
		SKP_assert(int64(sum>>1) == int64(SKP_SAT16(sum>>1)))
		sum = sum >> 1
		SKP_assert(int64((-i)<<4) == int64(SKP_SAT16((-i)<<4)))
		sum = SKP_SMLAWB(sum, sum, (-i)<<4)
		SKP_assert(int64(sum) == int64(SKP_SAT16(sum)))
		C[0][i] = int16(sum)
	}
	length_d_srch = complexity*2 + 4
	SKP_assert(length_d_srch*3 <= PITCH_EST_D_SRCH_LENGTH)
	...
	for k = 0; k < PITCH_EST_NB_SUBFR; k++ {
		SKP_assert(uintptr(unsafe.Pointer(&target_ptr[0])) >= uintptr(unsafe.Pointer(&signal_8kHz[0])))
		SKP_assert(uintptr(unsafe.Pointer(&target_ptr[sf_length_8kHz])) <= uintptr(unsafe.Pointer(&signal_8kHz[frame_length_8kHz])))
		...
		for j = 0; j < length_d_comp; j++ {
			...
			SKP_assert(uintptr(unsafe.Pointer(&basis_ptr[0])) >= uintptr(unsafe.Pointer(&signal_8kHz[0])))
			SKP_assert(uintptr(unsafe.Pointer(&basis_ptr[sf_length_8kHz])) <= uintptr(unsafe.Pointer(&signal_8kHz[frame_length_8kHz])))
			cross_corr = SKP_Silk_inner_prod_aligned(target_ptr, basis_ptr, sf_length_8kHz)
			...
			if cross_corr > 0 {
				...
				SKP_assert(int64(temp32) == int64(SKP_SAT16(temp32)))
	...
	SKP_assert(int64(search_thres2_Q15) == int64(SKP_SAT16(search_thres2_Q15)))
	...
	for k = 0; k < length_d_srch; k++ {
		...
		SKP_assert(int64(lag_log2_Q7) == int64(SKP_SAT16(lag_log2_Q7)))
		...
		if prevLag > 0 {
			...
			SKP_assert(int64(delta_lag_log2_sqr_Q7) == int64(SKP_SAT16(delta_lag_log2_sqr_Q7)))
	...
	if Fs_kHz > 8 {
		...
		CBimax_old = CBimax
		SKP_assert(int64(lag) == int64(SKP_SAT16(lag)))
		if Fs_kHz == 12 {
		...
		for d = start_lag; d <= end_lag; d++ {
			for j = cbk_offset; j < (cbk_offset + cbk_size); j++ {
				...
				if cross_corr > 0 {
					...
					SKP_assert(int64(diff) == int64(SKP_SAT16(diff)))
	...
	for k = 0; k < PITCH_EST_NB_SUBFR; k++ {
		...
		for i = cbk_offset; i < (cbk_offset + cbk_size); i++ {
			...
			for j = 0; j < PITCH_EST_NB_STAGE3_LAGS; j++ {
				...
				SKP_assert(float64(energies_st3[k][i][j]) >= 0.0)
			}
		}
		target_ptr += ([]int16)(sf_length)
	}
