...
func SKP_Silk_NSQ_del_dec(psEncC *SKP_Silk_encoder_state, psEncCtrlC *SKP_Silk_encoder_control, NSQ *SKP_Silk_nsq_state, x []int16, q []int8, LSFInterpFactor_Q2 int32, PredCoef_Q12 [32]int16, LTPCoef_Q14 [20]int16, AR2_Q13 [64]int16, HarmShapeGain_Q14 [4]int32, Tilt_Q14 [4]int32, LF_shp_Q14 [4]int32, Gains_Q16 [4]int32, Lambda_Q10 int32, LTP_scale_Q14 int32) {
	...
	subfr_length = int32(int64(psEncC.Frame_length) / NB_SUBFR)
	lag = NSQ.LagPrev
	memset(unsafe.Pointer(&psDelDec[0]), 0, size_t(uintptr(psEncC.NStatesDelayedDecision)*unsafe.Sizeof(NSQ_del_dec_struct{})))
	for k = 0; int64(k) < int64(psEncC.NStatesDelayedDecision); k++ {
		psDD = &psDelDec[k]
		psDD.Seed = int32((int64(k) + int64(psEncCtrlC.Seed)) & 3)
		psDD.SeedInit = psDD.Seed
		psDD.RD_Q10 = 0
		psDD.LF_AR_Q12 = NSQ.SLF_AR_shp_Q12
		psDD.Shape_Q10[0] = NSQ.SLTP_shp_Q10[int64(psEncC.Frame_length)-1]
	...
	if int64(psEncCtrlC.Sigtype) == SIG_TYPE_VOICED {
		for k = 0; int64(k) < NB_SUBFR; k++ {
			decisionDelay = SKP_min_int(decisionDelay, int32(int64(psEncCtrlC.PitchL[k])-LTP_ORDER/2-1))
		}
	} else {
		if int64(lag) > 0 {
			decisionDelay = SKP_min_int(decisionDelay, int32(int64(lag)-LTP_ORDER/2-1))
		}
	}
		...
		if int64(psEncCtrlC.Sigtype) == SIG_TYPE_VOICED {
			...
			if (int64(k) & (3 - (int64(LSF_interpolation_flag) << 1))) == 0 {
				if int64(k) == 2 {
					...
					Winner_ind = 0
					for i = 1; int64(i) < int64(psEncC.NStatesDelayedDecision); i++ {
						if int64(psDelDec[i].RD_Q10) < int64(RDmin_Q10) {
							RDmin_Q10 = psDelDec[i].RD_Q10
							Winner_ind = i
						}
					}
					for i = 0; int64(i) < int64(psEncC.NStatesDelayedDecision); i++ {
						if int64(i) != int64(Winner_ind) {
							psDelDec[i].RD_Q10 += SKP_int32_MAX >> 4
						}
					}
					psDD = &psDelDec[Winner_ind]
					last_smple_idx = int32(int64(smpl_buf_idx) + int64(decisionDelay))
					for i = 0; int64(i) < int64(decisionDelay); i++ {
						last_smple_idx = int32((int64(last_smple_idx) - 1) & (DECISION_DELAY - 1))
						q[int64(i)-int64(decisionDelay)] = int8(int64(psDD.Q_Q10[last_smple_idx]) >> 10)
						*(*int16)(unsafe.Add(unsafe.Pointer(pxq), unsafe.Sizeof(int16(0))*uintptr(int64(i)-int64(decisionDelay)))) = SKP_SAT16(int16(SKP_RSHIFT_ROUND(SKP_SMULWW(psDD.Xq_Q10[last_smple_idx], psDD.Gain_Q16[last_smple_idx]), 10)))
						NSQ.SLTP_shp_Q10[int64(NSQ.SLTP_shp_buf_idx)-int64(decisionDelay)+int64(i)] = psDD.Shape_Q10[last_smple_idx]
					}
					subfr = 0
				}
				...
				SKP_Silk_MA_Prediction(&NSQ.Xq[int64(start_idx)+int64(k)*int64(psEncC.Subfr_length)], A_Q12, &FiltState[0], &sLTP[start_idx], int32(int64(psEncC.Frame_length)-int64(start_idx)), psEncC.PredictLPCOrder)
				NSQ.SLTP_buf_idx = psEncC.Frame_length
				NSQ.Rewhite_flag = 1
			}
		}
	...
	RDmin_Q10 = psDelDec[0].RD_Q10
	Winner_ind = 0
	for k = 1; int64(k) < int64(psEncC.NStatesDelayedDecision); k++ {
		if int64(psDelDec[k].RD_Q10) < int64(RDmin_Q10) {
	...
	for i = 0; int64(i) < int64(decisionDelay); i++ {
		...
		*(*int16)(unsafe.Add(unsafe.Pointer(pxq), unsafe.Sizeof(int16(0))*uintptr(int64(i)-int64(decisionDelay)))) = SKP_SAT16(int16(SKP_RSHIFT_ROUND(SKP_SMULWW(psDD.Xq_Q10[last_smple_idx], psDD.Gain_Q16[last_smple_idx]), 10)))
		NSQ.SLTP_shp_Q10[int64(NSQ.SLTP_shp_buf_idx)-int64(decisionDelay)+int64(i)] = psDD.Shape_Q10[last_smple_idx]
		sLTP_Q16[int64(NSQ.SLTP_buf_idx)-int64(decisionDelay)+int64(i)] = psDD.Pred_Q16[last_smple_idx]
	}
...
func SKP_Silk_noise_shape_quantizer_del_dec(NSQ *SKP_Silk_nsq_state, psDelDec []NSQ_del_dec_struct, sigtype int32, x_Q10 []int32, q []int8, xq []int16, sLTP_Q16 []int32, a_Q12 []int16, b_Q14 []int16, AR_shp_Q13 []int16, lag int32, HarmShapeFIRPacked_Q14 int32, Tilt_Q14 int32, LF_shp_Q14 int32, Gain_Q16 int32, Lambda_Q10 int32, offset_Q10 int32, length int32, subfr int32, shapingLPCOrder int32, predictLPCOrder int32, warping_Q16 int32, nStatesDelayedDecision int32, smpl_buf_idx *int32, decisionDelay int32) {
	...
	for i = 0; int64(i) < int64(length); i++ {
		...
		for k = 0; int64(k) < int64(nStatesDelayedDecision); k++ {
			psDD = &psDelDec[k]
			psSS = &psSampleState[k][0]
			psDD.Seed = int32(int64(uint32(int32(int64(uint32(psDD.Seed))*0xBB38435))) + 0x3619636B)
			dither = psDD.Seed >> 31
			psLPC_Q14 = &psDD.SLPC_Q14[DECISION_DELAY-1+int64(i)]
			...
			tmp1 = SKP_SMLAWB(psDD.SAR2_Q14[0], int32(int64(psDD.SAR2_Q14[1])-int64(tmp2)), warping_Q16)
			psDD.SAR2_Q14[0] = tmp2
			n_AR_Q10 = SKP_SMULWB(tmp2, int32(AR_shp_Q13[0]))
			for j = 2; int64(j) < int64(shapingLPCOrder); j += 2 {
				tmp2 = SKP_SMLAWB(psDD.SAR2_Q14[int64(j)-1], int32(int64(psDD.SAR2_Q14[int64(j)+0])-int64(tmp1)), warping_Q16)
				psDD.SAR2_Q14[int64(j)-1] = tmp1
				n_AR_Q10 = SKP_SMLAWB(n_AR_Q10, tmp1, int32(AR_shp_Q13[int64(j)-1]))
				tmp1 = SKP_SMLAWB(psDD.SAR2_Q14[int64(j)+0], int32(int64(psDD.SAR2_Q14[int64(j)+1])-int64(tmp2)), warping_Q16)
				psDD.SAR2_Q14[int64(j)+0] = tmp2
				n_AR_Q10 = SKP_SMLAWB(n_AR_Q10, tmp2, int32(AR_shp_Q13[j]))
			}
			psDD.SAR2_Q14[int64(shapingLPCOrder)-1] = tmp1
			...
			n_LF_Q10 = int32(int64(SKP_SMULWB(psDD.Shape_Q10[*smpl_buf_idx], LF_shp_Q14)) << 2)
			...
			r_Q10 = int32(int64(x_Q10[i]) - int64(tmp1))
			...
			if int64(r_Q10) < -1536 {
				q1_Q10 = int32(int64(SKP_RSHIFT_ROUND(r_Q10, 10)) << 10)
				r_Q10 = int32(int64(r_Q10) - int64(q1_Q10))
				rd1_Q10 = SKP_SMLABB(int32(int64(int32(-(int64(q1_Q10)+int64(offset_Q10))))*int64(Lambda_Q10)), r_Q10, r_Q10) >> 10
				rd2_Q10 = int32(int64(rd1_Q10) + 1024)
				rd2_Q10 = int32(int64(rd2_Q10) - (int64(Lambda_Q10) + (int64(r_Q10) << 1)))
				q2_Q10 = int32(int64(q1_Q10) + 1024)
			} else if int64(r_Q10) > 512 {
				q1_Q10 = int32(int64(SKP_RSHIFT_ROUND(r_Q10, 10)) << 10)
				r_Q10 = int32(int64(r_Q10) - int64(q1_Q10))
				rd1_Q10 = SKP_SMLABB(int32((int64(q1_Q10)+int64(offset_Q10))*int64(Lambda_Q10)), r_Q10, r_Q10) >> 10
				rd2_Q10 = int32(int64(rd1_Q10) + 1024)
				rd2_Q10 = int32(int64(rd2_Q10) - (int64(Lambda_Q10) - (int64(r_Q10) << 1)))
				q2_Q10 = int32(int64(q1_Q10) - 1024)
			} else {
				...
				rd1_Q10 = int32(int64(rd2_Q10) + 1024)
				rd1_Q10 = int32(int64(rd1_Q10) + ((int64(Lambda_Q10) + (int64(r_Q10) << 1)) - (int64(rr_Q20) >> 9)))
				q1_Q10 = -1024
				q2_Q10 = 0
			}
			if int64(rd1_Q10) < int64(rd2_Q10) {
				(*(*NSQ_sample_struct)(unsafe.Add(unsafe.Pointer(psSS), unsafe.Sizeof(NSQ_sample_struct{})*0))).RD_Q10 = int32(int64(psDD.RD_Q10) + int64(rd1_Q10))
				(*(*NSQ_sample_struct)(unsafe.Add(unsafe.Pointer(psSS), unsafe.Sizeof(NSQ_sample_struct{})*1))).RD_Q10 = int32(int64(psDD.RD_Q10) + int64(rd2_Q10))
				...
				(*(*NSQ_sample_struct)(unsafe.Add(unsafe.Pointer(psSS), unsafe.Sizeof(NSQ_sample_struct{})*0))).RD_Q10 = int32(int64(psDD.RD_Q10) + int64(rd2_Q10))
				(*(*NSQ_sample_struct)(unsafe.Add(unsafe.Pointer(psSS), unsafe.Sizeof(NSQ_sample_struct{})*1))).RD_Q10 = int32(int64(psDD.RD_Q10) + int64(rd1_Q10))
			...
			exc_Q10 = int32(int64(offset_Q10) + int64((*(*NSQ_sample_struct)(unsafe.Add(unsafe.Pointer(psSS), unsafe.Sizeof(NSQ_sample_struct{})*0))).Q_Q10))
			...
			(*(*NSQ_sample_struct)(unsafe.Add(unsafe.Pointer(psSS), unsafe.Sizeof(NSQ_sample_struct{})*0))).SLTP_shp_Q10 = int32(int64(sLF_AR_shp_Q10) - int64(n_LF_Q10))
			(*(*NSQ_sample_struct)(unsafe.Add(unsafe.Pointer(psSS), unsafe.Sizeof(NSQ_sample_struct{})*0))).LF_AR_Q12 = int32(int64(sLF_AR_shp_Q10) << 2)
			(*(*NSQ_sample_struct)(unsafe.Add(unsafe.Pointer(psSS), unsafe.Sizeof(NSQ_sample_struct{})*0))).Xq_Q14 = int32(int64(xq_Q10) << 4)
			(*(*NSQ_sample_struct)(unsafe.Add(unsafe.Pointer(psSS), unsafe.Sizeof(NSQ_sample_struct{})*0))).LPC_exc_Q16 = int32(int64(LPC_exc_Q10) << 6)
			exc_Q10 = int32(int64(offset_Q10) + int64((*(*NSQ_sample_struct)(unsafe.Add(unsafe.Pointer(psSS), unsafe.Sizeof(NSQ_sample_struct{})*1))).Q_Q10))
			exc_Q10 = int32((int64(exc_Q10) ^ int64(dither)) - int64(dither))
			LPC_exc_Q10 = int32(int64(exc_Q10) + int64(SKP_RSHIFT_ROUND(LTP_pred_Q14, 4)))
			xq_Q10 = int32(int64(LPC_exc_Q10) + int64(LPC_pred_Q10))
			sLF_AR_shp_Q10 = int32(int64(xq_Q10) - int64(n_AR_Q10))
			(*(*NSQ_sample_struct)(unsafe.Add(unsafe.Pointer(psSS), unsafe.Sizeof(NSQ_sample_struct{})*1))).SLTP_shp_Q10 = int32(int64(sLF_AR_shp_Q10) - int64(n_LF_Q10))
			(*(*NSQ_sample_struct)(unsafe.Add(unsafe.Pointer(psSS), unsafe.Sizeof(NSQ_sample_struct{})*1))).LF_AR_Q12 = int32(int64(sLF_AR_shp_Q10) << 2)
			(*(*NSQ_sample_struct)(unsafe.Add(unsafe.Pointer(psSS), unsafe.Sizeof(NSQ_sample_struct{})*1))).Xq_Q14 = int32(int64(xq_Q10) << 4)
			(*(*NSQ_sample_struct)(unsafe.Add(unsafe.Pointer(psSS), unsafe.Sizeof(NSQ_sample_struct{})*1))).LPC_exc_Q16 = int32(int64(LPC_exc_Q10) << 6)
		}
		*smpl_buf_idx = int32((int64(*smpl_buf_idx) - 1) & (DECISION_DELAY - 1))
		last_smple_idx = int32((int64(*smpl_buf_idx) + int64(decisionDelay)) & (DECISION_DELAY - 1))
		RDmin_Q10 = psSampleState[0][0].RD_Q10
		Winner_ind = 0
		for k = 1; int64(k) < int64(nStatesDelayedDecision); k++ {
			if int64(psSampleState[k][0].RD_Q10) < int64(RDmin_Q10) {
		...
		for k = 0; int64(k) < int64(nStatesDelayedDecision); k++ {
			if int64(psDelDec[k].RandState[last_smple_idx]) != int64(Winner_rand_state) {
				psSampleState[k][0].RD_Q10 = int32(int64(psSampleState[k][0].RD_Q10) + (SKP_int32_MAX >> 4))
				psSampleState[k][1].RD_Q10 = int32(int64(psSampleState[k][1].RD_Q10) + (SKP_int32_MAX >> 4))
			}
		}
		...
		RDmin_ind = 0
		for k = 1; int64(k) < int64(nStatesDelayedDecision); k++ {
			if int64(psSampleState[k][0].RD_Q10) > int64(RDmax_Q10) {
				RDmax_Q10 = psSampleState[k][0].RD_Q10
				RDmax_ind = k
			}
			if int64(psSampleState[k][1].RD_Q10) < int64(RDmin_Q10) {
				RDmin_Q10 = psSampleState[k][1].RD_Q10
				RDmin_ind = k
			}
		}
		if int64(RDmin_Q10) < int64(RDmax_Q10) {
		...
		psDD = &psDelDec[Winner_ind]
		if int64(subfr) > 0 || int64(i) >= int64(decisionDelay) {
			q[int64(i)-int64(decisionDelay)] = int8(int64(psDD.Q_Q10[last_smple_idx]) >> 10)
			xq[int64(i)-int64(decisionDelay)] = SKP_SAT16(int16(SKP_RSHIFT_ROUND(SKP_SMULWW(psDD.Xq_Q10[last_smple_idx], psDD.Gain_Q16[last_smple_idx]), 10)))
			NSQ.SLTP_shp_Q10[int64(NSQ.SLTP_shp_buf_idx)-int64(decisionDelay)] = psDD.Shape_Q10[last_smple_idx]
			sLTP_Q16[int64(NSQ.SLTP_buf_idx)-int64(decisionDelay)] = psDD.Pred_Q16[last_smple_idx]
		}
		...
		for k = 0; int64(k) < int64(nStatesDelayedDecision); k++ {
			...
			psDD.SLPC_Q14[DECISION_DELAY+int64(i)] = psSS.Xq_Q14
			...
			psDD.Seed = int32(int64(psDD.Seed) + (int64(psSS.Q_Q10) >> 10))
