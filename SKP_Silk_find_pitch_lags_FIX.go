...
func SKP_Silk_find_pitch_lags_FIX(psEnc *SKP_Silk_encoder_state_FIX, psEncCtrl *SKP_Silk_encoder_control_FIX, res []int16, x []int16) {
	...
	buf_len = int32(int64(psEnc.SCmn.La_pitch) + (int64(psEnc.SCmn.Frame_length) << 1))
	...
	memcpy(unsafe.Pointer(Wsig_ptr), unsafe.Pointer(x_buf_ptr), size_t((int64(psPredSt.Pitch_LPC_win_length)-(int64(psEnc.SCmn.La_pitch)<<1))*int64(unsafe.Sizeof(int16(0)))))
	Wsig_ptr = (*int16)(unsafe.Add(unsafe.Pointer(Wsig_ptr), unsafe.Sizeof(int16(0))*uintptr(int64(psPredSt.Pitch_LPC_win_length)-(int64(psEnc.SCmn.La_pitch)<<1))))
	x_buf_ptr = (*int16)(unsafe.Add(unsafe.Pointer(x_buf_ptr), unsafe.Sizeof(int16(0))*uintptr(int64(psPredSt.Pitch_LPC_win_length)-(int64(psEnc.SCmn.La_pitch)<<1))))
	SKP_Silk_apply_sine_window(([]int16)(Wsig_ptr), ([]int16)(x_buf_ptr), 2, psEnc.SCmn.La_pitch)
	SKP_Silk_autocorr(auto_corr[:], &scale, Wsig[:], psPredSt.Pitch_LPC_win_length, int32(int64(psEnc.SCmn.PitchEstimationLPCOrder)+1))
	...
	for i = 0; int64(i) < int64(psEnc.SCmn.PitchEstimationLPCOrder); i++ {
		A_Q12[i] = SKP_SAT16(int16(int64(A_Q24[i]) >> 12))
	}
	...
	thrhld_Q15 = int32(SKP_SAT16(int16(thrhld_Q15)))
