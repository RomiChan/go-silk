...
func SKP_Silk_find_pitch_lags_FIX(psEnc *SKP_Silk_encoder_state_FIX, psEncCtrl *SKP_Silk_encoder_control_FIX, res []int16, x []int16) {
	...
	SKP_Silk_bwexpander(A_Q12[:], psEnc.SCmn.PitchEstimationLPCOrder, SKP_FIX_CONST(0.99, 16))
	memset(unsafe.Pointer(&FiltState[0]), 0, size_t(uintptr(psEnc.SCmn.PitchEstimationLPCOrder)*unsafe.Sizeof(int32(0))))
	SKP_Silk_MA_Prediction(([]int16)(x_buf), A_Q12[:], FiltState[:], res, buf_len, psEnc.SCmn.PitchEstimationLPCOrder)
	...
	psEncCtrl.SCmn.Sigtype = SKP_Silk_pitch_analysis_core(res, psEncCtrl.SCmn.PitchL[:], &psEncCtrl.SCmn.LagIndex, &psEncCtrl.SCmn.ContourIndex, &psEnc.LTPCorr_Q15, psEnc.SCmn.PrevLag, psEnc.SCmn.PitchEstimationThreshold_Q16, int32(int16(thrhld_Q15)), psEnc.SCmn.Fs_kHz, psEnc.SCmn.PitchEstimationComplexity, SKP_FALSE)
}
