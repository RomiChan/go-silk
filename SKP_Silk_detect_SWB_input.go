...
func SKP_Silk_detect_SWB_input(psSWBdetect *SKP_Silk_detect_SWB_state, samplesIn []int16, nSamplesIn int32) {
	...
	if int64(energy_32) > (int64(SKP_SMULBB(HP_8_KHZ_THRES, HP_8_kHz_len)) >> int64(shift)) {
		psSWBdetect.ConsecSmplsAboveThres += nSamplesIn
		if int64(psSWBdetect.ConsecSmplsAboveThres) > 480*15 {
			psSWBdetect.SWB_detected = 1
		}
	} else {
		psSWBdetect.ConsecSmplsAboveThres -= nSamplesIn
		if int64(psSWBdetect.ConsecSmplsAboveThres) > 0 {
			psSWBdetect.ConsecSmplsAboveThres = psSWBdetect.ConsecSmplsAboveThres
		} else {
			psSWBdetect.ConsecSmplsAboveThres = 0
		}
	}
	if int64(psSWBdetect.ActiveSpeech_ms) > WB_DETECT_ACTIVE_SPEECH_MS_THRES && int64(psSWBdetect.SWB_detected) == 0 {
