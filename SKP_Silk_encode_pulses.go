...
func combine_and_check(pulses_comb *int32, pulses_in *int32, max_pulses int32, len_ int32) int32 {
	...
	for k = 0; int64(k) < int64(len_); k++ {
		sum = int32(int64(*(*int32)(unsafe.Add(unsafe.Pointer(pulses_in), unsafe.Sizeof(int32(0))*uintptr(int64(k)*2)))) + int64(*(*int32)(unsafe.Add(unsafe.Pointer(pulses_in), unsafe.Sizeof(int32(0))*uintptr(int64(k)*2+1)))))
		if int64(sum) > int64(max_pulses) {
			return 1
		}
...
func SKP_Silk_encode_pulses(psRC *SKP_Silk_range_coder_state, sigtype int32, QuantOffsetType int32, q []int8, frame_length int32) {
	...
	iter = int32(int64(frame_length) / SHELL_CODEC_FRAME_LENGTH)
	for i = 0; int64(i) < int64(frame_length); i += 4 {
		abs_pulses[int64(i)+0] = int32(SKP_abs(int64(q[int64(i)+0])))
		abs_pulses[int64(i)+1] = int32(SKP_abs(int64(q[int64(i)+1])))
		abs_pulses[int64(i)+2] = int32(SKP_abs(int64(q[int64(i)+2])))
		abs_pulses[int64(i)+3] = int32(SKP_abs(int64(q[int64(i)+3])))
	}
	abs_pulses_ptr = &abs_pulses[0]
	for i = 0; int64(i) < int64(iter); i++ {
		nRshifts[i] = 0
		for {
			...
			sum_pulses[i] = int32(int64(pulses_comb[0]) + int64(pulses_comb[1]))
			if int64(sum_pulses[i]) > int64(SKP_Silk_max_pulses_table[3]) {
				scale_down++
			}
			if int64(scale_down) != 0 {
				nRshifts[i]++
				for k = 0; int64(k) < SHELL_CODEC_FRAME_LENGTH; k++ {
	...
	for k = 0; int64(k) < N_RATE_LEVELS-1; k++ {
		...
		if int64(sumBits_Q6) < int64(minSumBits_Q6) {
	...
	for i = 0; int64(i) < int64(iter); i++ {
		if int64(nRshifts[i]) == 0 {
			...
			for k = 0; int64(k) < int64(nRshifts[i])-1; k++ {
	...
	for i = 0; int64(i) < int64(iter); i++ {
		if int64(sum_pulses[i]) > 0 {
			SKP_Silk_shell_encoder(psRC, ([]int32)(&abs_pulses[int64(i)*SHELL_CODEC_FRAME_LENGTH]))
		}
	}
	for i = 0; int64(i) < int64(iter); i++ {
		if int64(nRshifts[i]) > 0 {
			pulses_ptr = &q[int64(i)*SHELL_CODEC_FRAME_LENGTH]
			nLS = int32(int64(nRshifts[i]) - 1)
			for k = 0; int64(k) < SHELL_CODEC_FRAME_LENGTH; k++ {
				abs_q = int32(int8(SKP_abs(int64(*(*int8)(unsafe.Add(unsafe.Pointer(pulses_ptr), k))))))
				for j = nLS; int64(j) > 0; j-- {
					bit = int32((int64(abs_q) >> int64(j)) & 1)
					SKP_Silk_range_encoder(psRC, bit, SKP_Silk_lsb_CDF[:])
				}
				bit = int32(int64(abs_q) & 1)
				SKP_Silk_range_encoder(psRC, bit, SKP_Silk_lsb_CDF[:])
			}
		}
	}
