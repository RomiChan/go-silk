...
func SKP_Silk_encode_frame_FIX(psEnc *SKP_Silk_encoder_state_FIX, pCode *uint8, pnBytesOut *int16, pIn *int16) int32 {
	...
	sEncCtrl.SCmn.Seed = int32(int64(func() int32 {
	...
	SKP_Silk_LP_variable_cutoff(&psEnc.SCmn.SLP, (*int16)(unsafe.Add(unsafe.Pointer(x_frame), unsafe.Sizeof(int16(0))*uintptr(LA_SHAPE_MS*int64(psEnc.SCmn.Fs_kHz)))), &pIn_HP[0], psEnc.SCmn.Frame_length)
	...
	if int64(psEnc.SCmn.NStatesDelayedDecision) > 1 || int64(psEnc.SCmn.Warping_Q16) > 0 {
	...
	if int64(psEnc.Speech_activity_Q8) < int64(SKP_FIX_CONST(SPEECH_ACTIVITY_DTX_THRES, 8)) {
		psEnc.SCmn.VadFlag = NO_VOICE_ACTIVITY
		psEnc.SCmn.NoSpeechCounter++
		if int64(psEnc.SCmn.NoSpeechCounter) > NO_SPEECH_FRAMES_BEFORE_DTX {
			psEnc.SCmn.InDTX = 1
		}
		if int64(psEnc.SCmn.NoSpeechCounter) > MAX_CONSECUTIVE_DTX+NO_SPEECH_FRAMES_BEFORE_DTX {
	...
	memmove(unsafe.Pointer(&psEnc.X_buf[0]), unsafe.Pointer(&psEnc.X_buf[psEnc.SCmn.Frame_length]), size_t((int64(psEnc.SCmn.Frame_length)+LA_SHAPE_MS*int64(psEnc.SCmn.Fs_kHz))*int64(unsafe.Sizeof(int16(0)))))
	psEnc.SCmn.Prev_sigtype = sEncCtrl.SCmn.Sigtype
	psEnc.SCmn.PrevLag = sEncCtrl.SCmn.PitchL[NB_SUBFR-1]
	psEnc.SCmn.First_frame_after_reset = 0
	if int64(psEnc.SCmn.SRC.Error) != 0 {
		psEnc.SCmn.NFramesInPayloadBuf = 0
	} else {
		psEnc.SCmn.NFramesInPayloadBuf++
	}
	if int64(psEnc.SCmn.NFramesInPayloadBuf)*FRAME_LENGTH_MS >= int64(psEnc.SCmn.PacketSize_ms) {
		LBRR_idx = int32((int64(psEnc.SCmn.Oldest_LBRR_idx) + 1) & LBRR_IDX_MASK)
		frame_terminator = SKP_SILK_LAST_FRAME
		if int64(psEnc.SCmn.LBRR_buffer[LBRR_idx].Usage) == SKP_SILK_ADD_LBRR_TO_PLUS1 {
			frame_terminator = SKP_SILK_LBRR_VER1
		}
		if int64(psEnc.SCmn.LBRR_buffer[psEnc.SCmn.Oldest_LBRR_idx].Usage) == SKP_SILK_ADD_LBRR_TO_PLUS2 {
		...
		if int64(*pnBytesOut) >= int64(nBytes) {
			...
			if int64(frame_terminator) > SKP_SILK_MORE_FRAMES && int64(*pnBytesOut) >= int64(nBytes)+int64(psEnc.SCmn.LBRR_buffer[LBRR_idx].NBytes) {
			...
			*pnBytesOut = int16(nBytes)
			...
			psEnc.SCmn.Oldest_LBRR_idx = int32((int64(psEnc.SCmn.Oldest_LBRR_idx) + 1) & LBRR_IDX_MASK)
		} else {
			*pnBytesOut = 0
			nBytes = 0
			ret = -4
	...
	psEnc.BufferedInChannel_ms += int32(((int64(nBytes) - int64(psEnc.SCmn.NBytesInPayloadBuf)) * (8 * 1000)) / int64(psEnc.SCmn.TargetRate_bps))
	...
	if int64(psEnc.Speech_activity_Q8) > int64(SKP_FIX_CONST(WB_DETECT_ACTIVE_SPEECH_LEVEL_THRES, 8)) {
...
func SKP_Silk_LBRR_encode_FIX(psEnc *SKP_Silk_encoder_state_FIX, psEncCtrl *SKP_Silk_encoder_control_FIX, pCode *uint8, pnBytesOut *int16, xfw []int16) {
	...
	if int64(psEnc.SCmn.LBRR_enabled) != 0 {
		...
		} else if int64(psEnc.SCmn.Fs_kHz) == 24 {
			Rate_only_parameters = 19500
		} else {
		}
		if int64(psEnc.SCmn.Complexity) > 0 && int64(psEnc.SCmn.TargetRate_bps) > int64(Rate_only_parameters) {
			if int64(psEnc.SCmn.NFramesInPayloadBuf) == 0 {
				...
				psEncCtrl.SCmn.GainsIndices[0] = int32(int64(psEncCtrl.SCmn.GainsIndices[0]) + int64(psEnc.SCmn.LBRR_GainIncreases))
		...
		if int64(psEnc.SCmn.SRC_LBRR.Error) != 0 {
			nFramesInPayloadBuf = 0
		} else {
			nFramesInPayloadBuf = int32(int64(psEnc.SCmn.NFramesInPayloadBuf) + 1)
		}
		if int64(SKP_SMULBB(nFramesInPayloadBuf, FRAME_LENGTH_MS)) >= int64(psEnc.SCmn.PacketSize_ms) {
			frame_terminator = SKP_SILK_LAST_FRAME
