		...
		for int64(inLen) > 0 {
			...
			if int64(S.NPreDownsamplers) > 0 {
				...
				if int64(S.NPostUpsamplers) > 0 {
					S.Resampler_function(S, out_buf, in_buf, int32(int64(nSamplesIn)>>int64(S.NPreDownsamplers)))
					S.Up_post_function(&S.SUpPost[0], &out[0], &out_buf[0], int32(int64(nSamplesOut)>>int64(S.NPostUpsamplers)))
				} else {
					S.Resampler_function(S, out, in_buf, int32(int64(nSamplesIn)>>int64(S.NPreDownsamplers)))
				}
			} else {
				S.Resampler_function(S, out_buf, in, int32(int64(nSamplesIn)>>int64(S.NPreDownsamplers)))
				S.Up_post_function(&S.SUpPost[0], out, out_buf, int32(int64(nSamplesOut)>>int64(S.NPostUpsamplers)))
			}
			in += ([]int16)(nSamplesIn)
