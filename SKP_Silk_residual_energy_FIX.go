...
func SKP_Silk_residual_energy_FIX(nrgs [4]int32, nrgsQ [4]int32, x []int16, a_Q12 [2][16]int16, gains [4]int32, subfr_length int32, LPC_order int32) {
	...
	for i = 0; i < 2; i++ {
		...
		SKP_Silk_LPC_analysis_filter(x_ptr, &a_Q12[i][0], &S[0], &LPC_res[0], (NB_SUBFR>>1)*offset, LPC_order)
		...
		x_ptr = (*int16)(unsafe.Add(unsafe.Pointer(x_ptr), unsafe.Sizeof(int16(0))*uintptr((NB_SUBFR>>1)*offset)))
	}
