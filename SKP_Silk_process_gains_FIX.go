...
func SKP_Silk_process_gains_FIX(psEnc *SKP_Silk_encoder_state_FIX, psEncCtrl *SKP_Silk_encoder_control_FIX) {
	...
	InvMaxSqrVal_Q16 = SKP_Silk_log2lin(SKP_SMULWB(SKP_FIX_CONST(70.0, 7)-psEncCtrl.Current_SNR_dB_Q7, SKP_FIX_CONST(0.33, 16))) / psEnc.SCmn.Subfr_length
	for k = 0; k < NB_SUBFR; k++ {
	...
	psEncCtrl.Lambda_Q10 = SKP_FIX_CONST(LAMBDA_OFFSET, 10) + SKP_SMULBB(SKP_FIX_CONST(-0.05, 10), psEnc.SCmn.NStatesDelayedDecision) + SKP_SMULWB(SKP_FIX_CONST(-0.3, 18), psEnc.Speech_activity_Q8) + SKP_SMULWB(SKP_FIX_CONST(-0.2, 12), psEncCtrl.Input_quality_Q14) + SKP_SMULWB(SKP_FIX_CONST(-0.1, 12), psEncCtrl.Coding_quality_Q14) + SKP_SMULWB(SKP_FIX_CONST(LAMBDA_QUANT_OFFSET, 16), quant_offset_Q10)
}
