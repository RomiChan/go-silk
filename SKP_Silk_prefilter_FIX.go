...
func SKP_Silk_warped_LPC_analysis_filter_FIX(state []int32, res []int16, coef_Q13 []int16, input []int16, lambda_Q16 int16, length int32, order int32) {
	...
	for n = 0; int64(n) < int64(length); n++ {
		...
		state[order] = tmp1
		acc_Q11 = SKP_SMLAWB(acc_Q11, tmp1, int32(coef_Q13[int64(order)-1]))
		res[n] = SKP_SAT16(int16(int64(input[n]) - int64(SKP_RSHIFT_ROUND(acc_Q11, 11))))
	}
}
func SKP_Silk_prefilter_FIX(psEnc *SKP_Silk_encoder_state_FIX, psEncCtrl *SKP_Silk_encoder_control_FIX, xw []int16, x []int16) {
	...
	for k = 0; int64(k) < NB_SUBFR; k++ {
		...
		HarmShapeGain_Q12 = SKP_SMULWB(psEncCtrl.HarmShapeGain_Q14[k], int32(0x4000-int64(psEncCtrl.HarmBoost_Q14[k])))
		HarmShapeFIRPacked_Q12 = HarmShapeGain_Q12 >> 2
		HarmShapeFIRPacked_Q12 |= int32(int64(int32(int64(HarmShapeGain_Q12)>>1)) << 16)
		Tilt_Q14 = psEncCtrl.Tilt_Q14[k]
		LF_shp_Q14 = psEncCtrl.LF_shp_Q14[k]
		AR1_shp_Q13 = &psEncCtrl.AR1_Q13[int64(k)*MAX_SHAPE_LPC_ORDER]
		...
		for j = 1; int64(j) < int64(psEnc.SCmn.Subfr_length); j++ {
			x_filt_Q12[j] = SKP_SMLABB(SKP_SMULBB(int32(st_res[j]), int32(B_Q12[0])), int32(st_res[int64(j)-1]), int32(B_Q12[1]))
		}
		P.SHarmHP = int32(st_res[int64(psEnc.SCmn.Subfr_length)-1])
...
func SKP_Silk_prefilt_FIX(P *SKP_Silk_prefilter_state_FIX, st_res_Q12 []int32, xw []int16, HarmShapeFIRPacked_Q12 int32, Tilt_Q14 int32, LF_shp_Q14 int32, lag int32, length int32) {
	...
	for i = 0; int64(i) < int64(length); i++ {
		if int64(lag) > 0 {
			idx = int32(int64(lag) + int64(LTP_shp_buf_idx))
			n_LTP_Q12 = SKP_SMULBB(int32(*(*int16)(unsafe.Add(unsafe.Pointer(LTP_shp_buf), unsafe.Sizeof(int16(0))*uintptr((int64(idx)-HARM_SHAPE_FIR_TAPS/2-1)&(LTP_BUF_LENGTH-1))))), HarmShapeFIRPacked_Q12)
			n_LTP_Q12 = SKP_SMLABT(n_LTP_Q12, int32(*(*int16)(unsafe.Add(unsafe.Pointer(LTP_shp_buf), unsafe.Sizeof(int16(0))*uintptr((int64(idx)-HARM_SHAPE_FIR_TAPS/2)&(LTP_BUF_LENGTH-1))))), HarmShapeFIRPacked_Q12)
			n_LTP_Q12 = SKP_SMLABB(n_LTP_Q12, int32(*(*int16)(unsafe.Add(unsafe.Pointer(LTP_shp_buf), unsafe.Sizeof(int16(0))*uintptr((int64(idx)-HARM_SHAPE_FIR_TAPS/2+1)&(LTP_BUF_LENGTH-1))))), HarmShapeFIRPacked_Q12)
		} else {
			n_LTP_Q12 = 0
		}
		...
		sLF_AR_shp_Q12 = int32(int64(st_res_Q12[i]) - (int64(n_Tilt_Q10) << 2))
		sLF_MA_shp_Q12 = int32(int64(sLF_AR_shp_Q12) - (int64(n_LF_Q10) << 2))
		LTP_shp_buf_idx = int32((int64(LTP_shp_buf_idx) - 1) & (LTP_BUF_LENGTH - 1))
		*(*int16)(unsafe.Add(unsafe.Pointer(LTP_shp_buf), unsafe.Sizeof(int16(0))*uintptr(LTP_shp_buf_idx))) = SKP_SAT16(int16(SKP_RSHIFT_ROUND(sLF_MA_shp_Q12, 12)))
		xw[i] = SKP_SAT16(int16(SKP_RSHIFT_ROUND(int32(int64(sLF_MA_shp_Q12)-int64(n_LTP_Q12)), 12)))
	}
