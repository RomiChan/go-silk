...
func SKP_Silk_find_pred_coefs_FIX(psEnc *SKP_Silk_encoder_state_FIX, psEncCtrl *SKP_Silk_encoder_control_FIX, res_pitch []int16) {
	...
	for i = 0; i < NB_SUBFR; i++ {
		if min_gain_Q16 < (psEncCtrl.Gains_Q16[i]) {
	...
	for i = 0; i < NB_SUBFR; i++ {
		...
		Wght_Q15[i] = tmp >> 1
		local_gains[i] = (1 << 16) / (invGains_Q16[i])
	}
	if psEncCtrl.SCmn.Sigtype == SIG_TYPE_VOICED {
		SKP_Silk_find_LTP_FIX(psEncCtrl.LTPCoef_Q14, WLTP, &psEncCtrl.LTPredCodGain_Q7, res_pitch, ([]int16)(&res_pitch[psEnc.SCmn.Frame_length>>1]), psEncCtrl.SCmn.PitchL, Wght_Q15, psEnc.SCmn.Subfr_length, psEnc.SCmn.Frame_length, LTP_corrs_rshift)
		...
		for i = 0; i < NB_SUBFR; i++ {
			SKP_Silk_scale_copy_vector16(x_pre_ptr, x_ptr, invGains_Q16[i], psEnc.SCmn.Subfr_length+psEnc.SCmn.PredictLPCOrder)
			x_pre_ptr = (*int16)(unsafe.Add(unsafe.Pointer(x_pre_ptr), unsafe.Sizeof(int16(0))*uintptr(psEnc.SCmn.Subfr_length+psEnc.SCmn.PredictLPCOrder)))
			x_ptr = (*int16)(unsafe.Add(unsafe.Pointer(x_ptr), unsafe.Sizeof(int16(0))*uintptr(psEnc.SCmn.Subfr_length)))
		}
		memset(unsafe.Pointer(&psEncCtrl.LTPCoef_Q14[0]), 0, size_t(NB_SUBFR*LTP_ORDER*unsafe.Sizeof(int16(0))))
		psEncCtrl.LTPredCodGain_Q7 = 0
	}
	SKP_Silk_find_LPC_FIX(NLSF_Q15[:], &psEncCtrl.SCmn.NLSFInterpCoef_Q2, psEnc.SPred.Prev_NLSFq_Q15[:], psEnc.SCmn.UseInterpolatedNLSFs*(1-psEnc.SCmn.First_frame_after_reset), psEnc.SCmn.PredictLPCOrder, LPC_in_pre[:], psEnc.SCmn.Subfr_length+psEnc.SCmn.PredictLPCOrder)
